...
def train_model_for_coin(df, coin):
    ...
    joblib.dump(X_scaler, f"{MODEL_DIR}/{coin}_x_scaler.pkl")
    joblib.dump(y_scaler, f"{MODEL_DIR}/{coin}_y_scaler.pkl")
...
def main():
    df = load_and_prepare_data("data/dataset_last.csv")
    coins = extract_coin_names(df)
    print(f"Moedas encontradas para retreinamento: {coins}")
    ...
    if results:
        ...
        results_df.to_csv(
            "data/models_lstm/lstm_model_retrained_mse.csv", index=False)
        print("\nResumo dos resultados:")
        print(results_df)
    else:
        print("Nenhum modelo foi retreinado com sucesso.")
