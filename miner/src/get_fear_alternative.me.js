...
const API_URL = `https://api.alternative.me/fng/?limit=9999999999999999999`;
...
async function recordExists(connection, timestamp) {
  const [rows] = await connection.execute(`
    SELECT 1
    FROM fear_index
    WHERE date = ? AND source = 'api.alternative.me'
    LIMIT 1
  `, [timestamp]);
...
async function insertData(connection, data) {
  ...
  for (const item of data) {
    ...
    if (convertedTimestamp && !(await recordExists(connection, convertedTimestamp))) {
      try {
        await connection.execute(insertQuery, ["api.alternative.me", null, convertedTimestamp, item.value]);
        insertedCount++;
      } catch (error) {
